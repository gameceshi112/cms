									...
									<table class="table no-more-tables m-0">
										...
										<tr>
											...
											<td class="{if $smarty.section.i.index mod 2 == 0}grey{else}white{/if}">												
												...
												<a class="btn btn-danger btn-xs btn-mini" href="videos.php?m=spam{if $page !=''}&page={$page}{/if}&a=delete&CID={$comments[i].CID}" onClick="javascript:return confirm('Are you sure you want to delete this comment?');">DELETE</a>
												<a class="btn btn-warning btn-xs btn-mini" href="videos.php?m=spam{if $page !=''}&page={$page}{/if}&a=unspam&SID={$comments[i].spam_id}" onClick="javascript:return confirm('Are you sure you want to unspam this comment?');">UNSPAM</a><br>
											</td>
										</tr>
										{/section}
										</tbody>
