    ...
    $("body").on('click', "button[id='add-photos-update']", function(event) {
		...
		$("input[id*='photo_added_caption_']").each(function () {
			...
			if (caption != '') {
				var captionData = {
							 id : photo_id,
						caption : caption.replace(/\r/g, "\\\\r").replace(/\t/g, "\\\\t")
				};
				var jsonCaptionData = JSON.stringify(captionData);
				$.post(base_url + '/ajax/admin_update_caption', {data: jsonCaptionData },
					function (response) {
						if (response.status) {
							//success message
						}
...
$(document).ready(function(){
	...
	$("body").on('click', "button[id='edit-save']", function(event) {		
		...
		if (!hasErrors("input[id*='edit-']") && !hasErrors("textarea[id*='edit-']")) {
			...
			var jsonAlbumData = JSON.stringify(albumData);
			
			$.post(base_url + '/ajax/admin_save_album', { data: jsonAlbumData },
				function (response) {					
	...
	$("body").on('click', "button[id='photo-edit-save']", function(event) {		
		...
		if (!hasErrors("input[id*='photo-edit-']")) {
			...
			var jsonPhotoData = JSON.stringify(photoData);
			
			$.post(base_url + '/ajax/admin_save_photo', { data: jsonPhotoData },
				function (response) {					
