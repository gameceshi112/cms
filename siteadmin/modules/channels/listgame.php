...
if ( isset($_POST['add_channel']) ) {
    ...
    if ( $name == '' ) {
        ...
        $sql        = "SELECT category_id FROM game_categories WHERE category_name = '" .mysql_real_escape_string($name). "' LIMIT 1";
        $conn->execute($sql);
	...
	if (channelSlugExists($slug, 0, 'game')) {
    ...
    if ( !$errors ) {
        $sql = "INSERT INTO game_categories (category_name, slug) VALUES ('" .mysql_real_escape_string($name). "', '".mysql_real_escape_string($slug)."')";
        $conn->execute($sql);
        $CID = $conn->Insert_ID();
		...
		if ( is_uploaded_file($_FILES['picture']['tmp_name']) ) {
			...
			if (in_array($ext, $exts)) {					
				...
				$dst    = $config['BASE_DIR']. '/media/categories/game/' .$CID. '.jpg';
...
function constructQuery()
{
    ...
    $query_count        = "SELECT count(category_id) AS total_channels FROM game_categories";
    $query_add          = NULL;
    $option_orig        = array('sort' => 'category_id', 'order' => 'DESC');
	
	$all   = (isset($_GET['all'])) ? intval($_GET['all']) : 0;
	if ($all == 1) {
		unset($_SESSION['search_gchannels_option']);
	}	
	
    $option             = ( isset($_SESSION['search_gchannels_option']) ) ? $_SESSION['search_gchannels_option'] : $option_orig;
    
    if ( isset($_POST['search_channels']) ) {
        $option['sort']     = trim($_POST['sort']);
        $option['order']    = trim($_POST['order']);      
        $_SESSION['search_gchannels_option'] = $option;
    }
