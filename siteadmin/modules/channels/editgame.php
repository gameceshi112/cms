...
if ( isset($_POST['edit_channel']) && !$errors ) {
    ...
    if ( $name == '' ) {
        ...
        $sql        = "SELECT category_id FROM game_categories
                       WHERE category_name = '" .mysql_real_escape_string($name). "' AND category_id != ".$CID." LIMIT 1";
        $conn->execute($sql);
    ...
    if ( !$errors ) {
        $sql = "UPDATE game_categories SET category_name = '" .mysql_real_escape_string($name). "', slug = '" .mysql_real_escape_string($slug). "' WHERE category_id = '" .mysql_real_escape_string($CID). "' LIMIT 1";
        $conn->execute($sql);
