...
if ( isset($_POST['add_channel']) ) {
    ...
    if ( $name == '' ) {
        ...
        $sql        = "SELECT CID FROM album_categories WHERE name = '" .mysql_real_escape_string($name). "' LIMIT 1";
        $conn->execute($sql);
    ...
    if ( !$errors ) {
        $sql = "INSERT INTO album_categories (name, slug) VALUES ('" .mysql_real_escape_string($name). "', '".mysql_real_escape_string($slug)."')";
        $conn->execute($sql);
        $CID = $conn->Insert_ID();
        ...
        if ( $errors ) {
            $sql = "DELETE FROM album_categories WHERE CID = '" .mysql_real_escape_string($CID). "' LIMIT 1";
            $conn->execute($sql);
        }
    }
