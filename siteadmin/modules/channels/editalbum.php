...
if ( isset($_POST['edit_channel']) && !$errors ) {
    ...
    if ( $name == '' ) {
        ...
        $sql        = "SELECT CID FROM album_categories
                       WHERE name = '" .mysql_real_escape_string($name). "' AND CID != ".$CID." LIMIT 1";
        $conn->execute($sql);
	...
	if (channelSlugExists($slug, $CID, 'album')) {
		$errors[]   = 'This slug already exists, please choose a different one!';
	}
	
    if ( !$errors ) {
        $sql = "UPDATE album_categories SET name = '" .mysql_real_escape_string($name). "', slug = '" .mysql_real_escape_string($slug). "' WHERE CID = '" .mysql_real_escape_string($CID). "' LIMIT 1";
        $conn->execute($sql);
...
$sql        = "SELECT * FROM album_categories WHERE CID = '" .mysql_real_escape_string($CID). "' LIMIT 1";
$rs         = $conn->execute($sql);
$channel    = $rs->getrows();
