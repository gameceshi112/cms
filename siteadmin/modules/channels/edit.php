...
if ( isset($_POST['edit_channel']) && !$errors ) {
    ...
    if ( $name == '' ) {
        ...
        $sql        = "SELECT CHID FROM channel
                       WHERE name = '" .mysql_real_escape_string($name). "' AND CHID != ".$CID." LIMIT 1";
        $conn->execute($sql);
    ...
    if ( !$errors ) {
        $sql = "UPDATE channel SET name = '" .mysql_real_escape_string($name). "', slug = '" .mysql_real_escape_string($slug). "' WHERE CHID = '" .mysql_real_escape_string($CID). "' LIMIT 1";
        $conn->execute($sql);
