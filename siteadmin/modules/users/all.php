...
function constructQuery($module)
{
    ...
    if ( isset($_POST['search_users']) ) {
        ...
        if ( $option['emailverified'] != '' ) {
            $query_option[] = $query_add. " emailverified = '" .mysql_real_escape_string($option['emailverified']). "'";
            $query_add      = " AND";            
        }

		if ( $option['status'] == 'Active' || $option['status'] == 'Inactive' ) {
            $query_option[] = $query_add. " account_status = '" .mysql_real_escape_string($option['status']). "'";
            $query_add      = " AND";   			
		}		

        if ( $option['premium'] != '' ) {
            $query_option[] = $query_add. " premium = '" .mysql_real_escape_string($option['premium']). "'";
            $query_add      = " AND";            
        }		
