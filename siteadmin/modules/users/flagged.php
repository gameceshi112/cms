...
function constructQuery()
{
    ...
    if ( isset($_POST['search_users']) ) {
        ...
        if ( $option['emailverified'] != '' ) {
            $query_option[] = " AND u.emailverified = '" .mysql_real_escape_string($option['emailverified']). "'";        
        }

		if ( $option['status'] == 'Active' || $option['status'] == 'Inactive' ) {
            $query_option[] = " AND u.account_status = '" .mysql_real_escape_string($option['status']). "'";			
		}		

        if ( $option['premium'] != '' ) {
            $query_option[] = " AND u.premium = '" .mysql_real_escape_string($option['premium']). "'";           
        }	
