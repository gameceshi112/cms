...
if ( isset($_POST['adv_add']) ) {
    ...
    if ( !$errors ) {
        $sql            = "INSERT INTO adv_pause (name, code, device, categories, status)
                           VALUES ('" .mysql_real_escape_string($adv['name']). "', 
                                   '" .mysql_real_escape_string($adv['code']). "', 
                                   '" .mysql_real_escape_string($adv['device']). "', 
                                   '" .mysql_real_escape_string($adv['categories']). "', 
								   '" .mysql_real_escape_string($adv['status']). "')";                          
        $conn->execute($sql);
