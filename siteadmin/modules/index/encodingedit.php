...
if ( isset($_POST['encoding_save']) && !$errors ) {
	...
	$sql        = "SELECT id FROM encoding WHERE label = '" .mysql_real_escape_string($encoding['label']). "' AND id != '" .mysql_real_escape_string($EID). "'LIMIT 1";
	$conn->execute($sql);
	if ( $conn->Affected_Rows() > 0 ) {
		$errors[]   = "Encoding label <b>" .mysql_real_escape_string($encoding['label'])."</b> is already used. Please choose a different label!";
		$err['label'] = 1;
	}
	...
	if ( !$errors ) {
		$sql	=	"UPDATE encoding SET
					label = '" .mysql_real_escape_string($encoding['label']). "', 
					width = '" .mysql_real_escape_string($encoding['width']). "', 
					height = '" .mysql_real_escape_string($encoding['height']). "', 
					crf = '" .mysql_real_escape_string($encoding['crf']). "', 
					preset = '" .mysql_real_escape_string($encoding['preset']). "', 
					ios = '" .mysql_real_escape_string($encoding['ios']). "', 
					faststart = '" .mysql_real_escape_string($encoding['faststart']). "', 
					format = '" .mysql_real_escape_string($encoding['format']). "', 
					copyonly = '" .mysql_real_escape_string($encoding['copyonly']). "', 
					status = '" .mysql_real_escape_string($encoding['status']). "'
					WHERE id = '" .mysql_real_escape_string($EID). "'LIMIT 1";
		$conn->execute($sql);
		$messages[]     = 'Encoding successfully updated!';
	}
}

$sql	=	"SELECT * FROM encoding WHERE id = '" .mysql_real_escape_string($EID). "' LIMIT 1";
