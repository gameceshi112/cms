	...
	if ( $_FILES['addthumb']['tmp_name'] != '' ) {		
		if ( is_uploaded_file($_FILES['addthumb']['tmp_name']) ) {
			...
			if (in_array($ext, $exts)) {					
				...
				$sql = "UPDATE signup SET photo = '" .mysql_real_escape_string($UID). ".jpg' WHERE UID = '" .mysql_real_escape_string($UID). "' LIMIT 1";
				$conn->execute($sql);
			}
		}
	}
} else {
	$sql = "UPDATE signup SET photo = '' WHERE UID = '" .mysql_real_escape_string($UID). "' LIMIT 1";
	$conn->execute($sql);
    $sql    = "SELECT gender FROM signup WHERE UID = '" .mysql_real_escape_string($UID). "' LIMIT 1";
    $rs     = $conn->execute($sql);
