...
switch ($type) {
    case 'Video':
			$sql = "UPDATE video_comments SET comment = '" .mysql_real_escape_string($comment). "'
				   WHERE CID = " .$id. " LIMIT 1";
			$conn->execute($sql);	
			$sql = "SELECT comment from video_comments 
				   WHERE CID = " .$id. " LIMIT 1";
			$rs = $conn->execute($sql);
			$response['comment'] = $rs->fields['comment'];
			$response['status'] = 1;			
        break;	
    case 'Photo':
			$sql = "UPDATE photo_comments SET comment = '" .mysql_real_escape_string($comment). "'
				   WHERE CID = " .$id. " LIMIT 1";
			$conn->execute($sql);	
			$sql = "SELECT comment from photo_comments 
				   WHERE CID = " .$id. " LIMIT 1";
			$rs = $conn->execute($sql);
			$response['comment'] = $rs->fields['comment'];
			$response['status'] = 1;		
        break;
    case 'Game':
			$sql = "UPDATE game_comments SET comment = '" .mysql_real_escape_string($comment). "'
				   WHERE CID = " .$id. " LIMIT 1";
			$conn->execute($sql);	
			$sql = "SELECT comment from game_comments 
				   WHERE CID = " .$id. " LIMIT 1";
			$rs = $conn->execute($sql);
			$response['comment'] = $rs->fields['comment'];
			$response['status'] = 1;	
        break;
    case 'Blog':
			$sql = "UPDATE blog_comments SET comment = '" .mysql_real_escape_string($comment). "'
				   WHERE CID = " .$id. " LIMIT 1";
			$conn->execute($sql);	
			$sql = "SELECT comment from blog_comments 
				   WHERE CID = " .$id. " LIMIT 1";
			$rs = $conn->execute($sql);
			$response['comment'] = $rs->fields['comment'];
			$response['status'] = 1;	
        break;
    case 'Notice':
			$sql = "UPDATE notice_comments SET comment = '" .mysql_real_escape_string($comment). "'
				   WHERE CID = " .$id. " LIMIT 1";
			$conn->execute($sql);	
			$sql = "SELECT comment from notice_comments 
				   WHERE CID = " .$id. " LIMIT 1";
			$rs = $conn->execute($sql);
			$response['comment'] = $rs->fields['comment'];
			$response['status'] = 1;	
