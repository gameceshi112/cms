...
switch ($type) {
    case 'Video':
			$sql = "DELETE FROM channel WHERE CHID = '" .mysql_real_escape_string($id). "' LIMIT 1";
			$conn->execute($sql);
        ...
        break;	
    case 'Album':
			$sql = "DELETE FROM album_categories WHERE CID = '" .mysql_real_escape_string($id). "' LIMIT 1";
			$conn->execute($sql);
			if ( $conn->Affected_Rows() ) {
				$response['status'] = 1;
        ...
        break;
    case 'Game':
			$sql = "DELETE FROM game_categories WHERE category_id = '" .mysql_real_escape_string($id). "' LIMIT 1";
			$conn->execute($sql);
			if ( $conn->Affected_Rows() ) {
				$response['status'] = 1;
        ...
        break;
    case 'Notice':
			$sql = "DELETE FROM notice_categories WHERE category_id = '" .mysql_real_escape_string($id). "' LIMIT 1";
			$conn->execute($sql);
			if ( $conn->Affected_Rows() ) {
				$response['status'] = 1;				
			}	
