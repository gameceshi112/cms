...
$sql  = "SELECT * FROM signup WHERE FBID = '" .mysql_real_escape_string($id). "' LIMIT 1";
$rs   = $conn->execute($sql);
$FBID = $rs->fields['FBID'];
if ($id == $FBID) {	
	...
	$sql    = "UPDATE signup SET logintime = '" .time(). "' WHERE FBID = '" .mysql_real_escape_string($FBID). "' LIMIT 1";
	$conn->execute($sql);
