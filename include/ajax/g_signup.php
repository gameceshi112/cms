...
unset($_SESSION['g_signup_email']);

$sql = "SELECT * FROM signup WHERE GID = '" .mysql_real_escape_string($id). "' LIMIT 1";
$rs  = $conn->execute($sql);
$GID = $rs->fields['GID'];
if ($id == $GID) {	
	...
	$sql    = "UPDATE signup SET logintime = '" .time(). "' WHERE GID = '" .mysql_real_escape_string($GID). "' LIMIT 1";
	$conn->execute($sql);
	...
	if ( $valid->emailExists($email) ) {
		$response['existing'] = 1;
		$sql = "SELECT * FROM signup WHERE email = '" .mysql_real_escape_string($email). "' LIMIT 1";
		$rs = $conn->execute($sql);
