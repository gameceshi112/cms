...
function get_user_playlist( $uid, $prefs, $is_friend, $limit=3 )
{
    ...
    if ( $show ) {
        global $conn;
        $sql        = "SELECT p.VID, v.thumb_img, v.title, v.duration, v.viewnumber, v.rate, v.addtime, v.type, v.thumb, v.thumbs, v.hd
...
function get_user_favorites( $uid, $prefs, $is_friend, $limit=3 )
{
    ...
    if ( $show ) {
        global $conn;
        $sql        = "SELECT f.VID, v.title, v.addtime, v.duration, v.thumb_img,v.viewnumber, v.rate, v.likes, v.dislikes, v.type, v.thumb, v.thumbs
...
{
    global $conn;
    $sql    = "SELECT VID, title, duration, viewnumber, rate, likes, dislikes, addtime, type, thumb,thumb_img, thumbs, hd
	           FROM video
