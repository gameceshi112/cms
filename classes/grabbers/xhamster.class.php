...
class VGrab_xhamster
{
	...
	function getVideoDescription() {
        preg_match('/description caption">(.*?)</', $this->page, $matches);
        if ( isset($matches['1']) ) {
    ...
    function getVideoCategory() {
	    if(preg_match_all('/href="\/categories\/(.*?)"/', $this->page, $matches_category)) {
    ...
    function getVideoUrl() {
		$vid = (int)end(explode('-',$this->url));
		$stream = $this->curl->saveToString("https://xhamster.com/xembed.php?video=".$vid);	
		preg_match('/sources:(.*?)}/', $stream, $matches);
		if ( isset($matches['1']) ) {
