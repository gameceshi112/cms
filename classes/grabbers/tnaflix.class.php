...
class VGrab_tnaflix
{
    ...
    function getVideoTitle() {
        ...
        if ( isset($matches['1']) ) {
			$title = trim(str_replace(' - tnaflix.com', '', $matches['1']));
    ...
    function getVideoUrl() {
		$content = $this->curl->saveToString("https://player.tnaflix.com/video/".$this->vkey);					
		preg_match('/config = "(.*?)"/', $content, $matches);
