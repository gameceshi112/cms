		...
		if ($html) {
			if (preg_match_all('/<li data-vid=(.*?)<\/li>/', $html, $matches)) {
				foreach ($matches[0] as $match) {
					...
					//Embed Code
					$video['embed'] = '<iframe src="https://player.empflix.com/video/' . $video['id'] . '" width="' . E_WIDTH . '" height="' . E_HEIGHT . '" frameborder="0"></iframe>';
					...
					if(preg_match('/href=\'(.*?)\'/', $match, $matches_url)) {
						$video['url']   = 'https://www.empflix.com'.$matches_url[1];
					} else {
					...
					//Title
					if(preg_match('/nvtHide\'>(.*?)</', $match, $matches_title)) {
                    ...
                    if(preg_match('/data-original=\'(.*?)\'/', $match, $matches_thumb)) {
						...
						for ($i = 1; $i <= 20; $i++) {
							$video['thumbs'][] = $thumb_url[0]."-".$i."l.jpg";
						}
					} else {
